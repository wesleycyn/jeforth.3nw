    ...
    : malloc    ( -- array ) \ Allocate an array
                ...
                <selftest>
					*** nop <== wut, explain the test strategy ... 
					js> [] js> stack isSameArray >r dropall r>
    ...
    code binary-array>string ( byte-array -- binary-string ) \ Convert an array into binary string
        var ba = pop(), bs=new Buffer(ba.length);
    ...
    : 8,        ( byte -- ) \ Compile the given binary byte into target space.
                ...
                target-here @ target-space js> pop()[pop()]=pop() drop \ drop the length
                target-here ++ ;
    ...
    : 16,       ( word -- ) \ Compile the given binary word into target space.
                ...
                <selftest>
					*** malloc target-space 8, 16, ++ org target-here ... 
					...
					js> stack js> [1,1,18,52,1,2,3] isSameArray >r dropall r>
    ...
    : poke8     ( byte addr -- ) \ Write a byte to target space
                swap 255 AND ( take care of negative numbers ) swap target-space js> pop()[pop()]=pop() drop ;

                <selftest>
					*** peek8 poke8 ... 
					1234567890 0xf poke8 0xf peek8
					js> [210] js> stack isSameArray >r dropall r>
    ...
    : poke16    ( word addr -- ) \ Write a word to target space
                ...
                <selftest>
					*** peek16 poke16 ... 
					...
					js> [0x7890] js> stack isSameArray >r dropall r>
    ...
    code cmove(t>t) ( fStart fEnd tStart -- ) \ move from (fStart-fEnd) to tStart, where fStart,fEnd,tStart are target addresses.
                ...
                <selftest>
					*** cmove(t>t) ... 
                    ...
                    0xfe peek8 \ 0x33
                    0xff peek8 \ 0x44
					js> [0x11,0x22,0x33,0x44] js> stack isSameArray >r dropall r>
...
\ -------------- Label tools ---------------------------------------------------------
    ...
    : L:        ( <label> -- ) \ Resolve all above 8rel> 16rel> labels. label is absolute address constant.
                create
                    char [ js> last().name char ] + + name>word ( word.name )
                    ?dup if ( word.name )
    ...
    : L>        ( bits <name> -- ) \ Forward refer to label "name".
                BL word ( bits "name" )
                \ dup name>word if cr ." Error! " . ."  re-defined." cr *debug* Error>> then ( bits "name" )
                char [ swap + char ] + ( bits "[name]" )
                dup name>word  ( bits "[name]" word["[name]"] )
                ?dup if ( bits "[name]" word["[name]"] )
                    nip target-here @ js> pop(1).storage.push(pop()) drop ( bits )
                else ( bits "[name]" )
                    (create) target-here @ js> last().storage=[];last().storage.push(pop()) drop ( bits )
                then ( bits )
                target-here @ 1- poke8 ;
	...
	L: LookForward  0x51 8, 0x52 8, 0x53 8, 
					LookForward 16, 0x63 8, 0x64 8,  8 L> LookForward
